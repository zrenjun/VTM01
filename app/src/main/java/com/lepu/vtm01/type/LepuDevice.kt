...
import com.lepu.vtm01.util.CmdUtil.trimStr
import com.lepu.vtm01.type.LepuBleResponse.bytesToHex

class LepuDevice (val bytes: ByteArray) {
    var hwV: Char            // 硬件版本
    var fwV: String          // 固件版本
    ...
    init {
        ...
        protocolMaxLen = toUInt(bytes.copyOfRange(31, 33))
        snLen = (bytes[37].toUInt() and 0xFFu).toInt()
        if (snLen > 18) {
            snLen = 18
        }
        sn = if ((38 + snLen) > bytes.size) {
            trimStr(String(bytes.copyOfRange(38, bytes.size)))
        } else {
            trimStr(String(bytes.copyOfRange(38, 38+ snLen)))
        }
    }
